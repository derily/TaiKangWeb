...
namespace TaiKang.Tests
{
    ...
    /// Provides methods to easily work with <see cref="TaiKangDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<TaiKangTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaiKangDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaiKangDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaiKangDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaiKangDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaiKangDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaiKangDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaiKangDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaiKangDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaiKangDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaiKangDbContext, Task<T>> func)
        {
            T result;
